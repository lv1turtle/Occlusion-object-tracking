...
class Tracker:
    ...
    def update(self, detections,
               ...
               only_extrapolate = False,
               extrapolated_iou_match = False,
               appearance_match = True,
               bugfix=True,
               tn=-1):
        
        start =  time.time()
        print("start tracker update------------------------------")
        ...
        # print("     [tracker update] _match", time.time()-start)
        # start = time.time()
        ...
        # print("     [tracker update] _ track.update", time.time()-start)
        # start = time.time()
        ...
        # print("     [tracker update] _ mark_occluded", time.time()-start)
        # start = time.time()
        ...
        # print("     [tracker update] _ mark_deleted", time.time()-start)
        # start = time.time()
        ...
        # print("     [tracker update] _ mark_missed", time.time()-start)
        # start = time.time()
        ...
        # print("     [tracker update] _ initiate_track", time.time()-start)
        # start = time.time()

        self.tracks = [t for t in self.tracks if not t.is_deleted()]
        # print("     [tracker update] _ is_deleted", time.time()-start)
        # start = time.time()
        ...
        # print("     [tracker update] _ update distance metric", time.time()-start)
        # start = time.time()
    ...
    def align(self, im1_gray, im2_gray):
        # print("shape", im1_gray.shape)
        # im1_gray = cv2.resize(im1_gray, dsize=(240, 135), interpolation=cv2.INTER_CUBIC)
        # im2_gray = cv2.resize(im2_gray, dsize=(240, 135), interpolation=cv2.INTER_CUBIC)
        ...
        criteria = (cv2.TERM_CRITERIA_EPS | cv2.TERM_CRITERIA_COUNT,
                    ...
                    termination_eps)
        print("criteria : ", time.time() - start)
        try:
            ...
            cc, warp_matrix = cv2.findTransformECC(im1_gray,
                    im2_gray, warp_matrix,
                    warp_mode, criteria)
        print("findTransformECC : ", time.time() - start)
        return warp_matrix
    ...
    def update_metadata(self, idx, path, seq_info, max_height,
        ...
        if not img_set:
            ...
            print("before warp_matrix : ", time.time() - start)
            self.tn = tn
            
            if self.frame_idx != 0:
                # print("=-------------------frame", self.frame_idx)
                # print("past_frame", self.past_frame)
                self.warp_matrix = self.align(self.past_frame,
                                        self.current_frame)
            print("after warp_matrix : ", time.time() - start)
        else :
            past = self.frame_idx - 2
            current = self.frame_idx - 1
            if self.frame_idx != 1:
                self.past_frame = seq_info["img_np"][past]
            self.current_frame = seq_info["img_np"][current]
            print("before warp_matrix : ", time.time() - start)
            self.tn = tn
