...
def gather_sequence_info(sequence_dir, detection_file, img_set):
    ...
    if img_set :
        img_set = os.path.join(sequence_dir, "img_set.npy") # add
        img_np = np.load(file=img_set) # add
...
def run(sequence_dir, detection_file, output_file, min_confidence,
    ...
    """
    seq_info = gather_sequence_info(sequence_dir, detection_file, img_set)
...
def parse_args():
    ...
    parser.add_argument(
        "--img_set", help="if you have a img_set",
        default=True, type=bool_string)
...
if __name__ == "__main__":
    ...
    for seq in sequences:
        ...
        if 'data' not in seq:
            continue
        detection_file = "/home/adriv/detect-invisible/detecting-invisible-people/resources/detections/MOT17_train/" + seq + ".npy"
        output_file = "/home/adriv/detect-invisible/detecting-invisible-people/results/" + args.output_file + "/{}.txt".format(seq)
        if os.path.exists(output_file):
            continue
        sequence_dir = args.sequence_dir + "/" + seq
        print(sequence_dir)
        depth_map_path = args.sequence_dir + "/" + seq
        run(
